...
use FTP\AdministradorBundle\Entity\sipproveedores\PedidosConsecutivos;
  ...
  */
  ...
  private function cargarAsociados(){
    ...
    foreach ($aux as $v){ 
        ...
        $asociados[ (int)$v['codigo'] ]['ciudad'] = str_replace(" ", "", $v['ciudad']);
...
//    echo "se termina de procesar el archivo $archivo \n";
    ...
    foreach ($drogueriasProcesadas as $drogueria){
      ...
      $pedidoConfirmado = $emsp->getRepository('SipproveedoresEntity:DescripcionPedido')->findBy(array('idPedido' => $pedido->getId() ));
                
      $consecutivoProducto = 1;
      foreach($pedidoConfirmado as $prodConf){

          $consecutivo = $drogueria['proveedor'].'-'.$pedido->getId()."-".$consecutivoProducto;
          $consecutivoPedido = new PedidosConsecutivos();

          $consecutivoPedido->setIdPedido($pedido->getId());
          $consecutivoPedido->setIdDescripcionPedido( $prodConf->getId() );
          $consecutivoPedido->setConsecutivo($consecutivo);

          $emsp->persist($consecutivoPedido);
          $emsp->flush();
